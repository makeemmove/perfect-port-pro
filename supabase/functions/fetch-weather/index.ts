...
type Alert = { event: string; severity: string; headline: string; description: string };
type DailyDay = { day: string; high: number; low: number; icon: string };
type WeatherPayload = {
  temp: number;
  precip: number;
  wind: number;
...
async function fetchFromApis(needForecast: boolean): Promise<{
  current: any;
  forecast: any;
  alerts: Alert[];
  sunrise: string;
  ...
  const [currentRes, forecastRes, alertsRes, sunRes] = await Promise.all([
    fetch(CURRENT_URL),
    needForecast ? fetch(FORECAST_URL) : Promise.resolve(null),
    fetch(ALERTS_URL, {
  ...
  const forecast = forecastRes && (forecastRes as Response).ok ? await (forecastRes as Response).json() : null;
  ...
  return { current, forecast, alerts, sunrise, sunset, daylight: daylightHrs };
}

function buildDaily(forecast: any): DailyDay[] {
  const daily: DailyDay[] = [];
  if (!forecast?.list) return daily;
...
serve(async (req) => {
  ...
  try {
    ...
    const type = url.searchParams.get("type") ?? "full"; // current | forecast | full

    const needForecast = type !== "current";
    const { current, forecast, alerts, sunrise, sunset, daylight } = await fetchFromApis(needForecast);
    const weather = buildWeatherPayload(current, forecast, alerts, sunrise, sunset, daylight);

    const supabaseUrl = Deno.env.get("SUPABASE_URL");
    const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
    const shouldWrite = supabaseUrl && supabaseKey;
    ...
    if (shouldWrite) {
      ...
      if (type === "current" || type === "full") {
        ...
        if (err) console.error("weather_current upsert error:", err);
      }

      if (type === "forecast" || type === "full") {
        const daily = weather.daily;
        await supabase.from("weather_forecast").delete().eq("location_id", LOCATION_ID);
        if (daily.length > 0) {
          const tz = "America/New_York";
          const formatter = new Intl.DateTimeFormat("en-CA", { timeZone: tz, year: "numeric", month: "2-digit", day: "2-digit" });
          const todayETStr = formatter.format(new Date()); // YYYY-MM-DD
          const rows = daily.map((d, i) => {
            const dte = new Date(todayETStr + "T12:00:00Z");
          ...
          const { error: err } = await supabase.from("weather_forecast").insert(rows);
          if (err) console.error("weather_forecast insert error:", err);
        }
      }
    }
