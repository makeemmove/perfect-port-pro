...
const Index = () => {
  const [activeTab, setActiveTab] = useState<'home' | 'eats' | 'events' | 'news' | 'community'>('home');
  ...
  return (
    <div className="flex flex-col h-[100dvh]">
      ...
      <div className="flex-1 overflow-hidden relative">
        {(['home', 'eats', 'events', 'news', 'community'] as const).map(tab => (
          <div
            key={tab}
          ...
          >
            <div className="max-w-lg mx-auto">
              {tab === 'home' && <HomeTab onNavigate={(t) => setActiveTab(t as any)} newsArticles={articles} onNewsClick={() => setActiveTab('news')} weather={weather} onCommunityClick={() => setActiveTab('community')} />}
              {tab === 'eats' && <EatsTab />}
