    ...
    <div className="space-y-2.5">
      ...
      <div className="flex flex-col gap-1.5">
        {articles.slice(0, 3).map((article, i) => (
          <div
            key={article.id}
            className="group flex items-center gap-3 p-3 rounded-[24px] bg-card shadow-card hover:shadow-card-hover hover:scale-[1.01] active:scale-[0.97] transition-all duration-150 cursor-pointer"
            onClick={() => onNewsClick?.()}
          >
      ...
      <button
        onClick={onNewsClick}
        className="w-full py-2.5 rounded-full bg-foreground/[0.03] text-foreground text-[11px] font-semibold tracking-wide uppercase shadow-card hover:shadow-card-hover active:scale-[0.97] transition-all duration-150"
      >
        View All News →
      </button>
    </div>
