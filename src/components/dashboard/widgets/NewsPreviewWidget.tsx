...
const NewsPreviewWidget = ({ articles, onNewsClick }: NewsPreviewWidgetProps) => {
  ...
  return (
    <div className="space-y-2.5">
      ...
      <button
        onClick={onNewsClick}
        className="w-full py-2.5 rounded-xl bg-foreground/[0.03] text-foreground text-[11px] font-semibold tracking-wide uppercase border border-border hover:bg-foreground/[0.06] hover:border-primary/20 transition-all"
      >
        View All News →
      </button>
    </div>
