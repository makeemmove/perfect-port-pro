...
const SortableEventItem = ({ id, event, onEventClick }: SortableEventItemProps) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-2 py-1.5 px-3.5 rounded-[24px] bg-card shadow-card hover:shadow-card-hover hover:scale-[1.01] active:scale-[0.98] transition-all duration-150 group/event"
    >
      <button
      ...
      {onEventClick && (
        <button
          ...
          className="text-[9px] font-semibold tracking-wide uppercase py-[2px] px-2 rounded-full bg-muted text-primary hover:bg-muted/80 transition-colors flex-shrink-0"
        >
          Info
        </button>
      )}
